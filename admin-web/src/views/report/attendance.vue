<template>
  <div class="report-page">
    <PageHeader title="考勤统计报表" subtitle="按班级出勤率 / 考勤状态聚合">
      <template #extra>
    ...
    <el-card>
      ...
      <div v-loading="loading">
        ...
        <el-table v-if="data" :data="data.byClass" border style="margin-top: 16px">
          ...
          <el-table-column label="总记录" prop="total" width="100" align="center" sortable />
          <el-table-column label="出勤" prop="present" width="90" align="center" />
          <el-table-column label="缺勤" prop="absent" width="90" align="center" />
          <el-table-column label="迟到" prop="late" width="90" align="center" />
          <el-table-column label="请假" prop="leave" width="90" align="center" />
...
import { getAttendanceReport, exportReport, type AttendanceReport } from '@/api/report';
...
async function loadData(): Promise<void> {
  loading.value = true;
  try {
    data.value = await getAttendanceReport(startDate.value || undefined, endDate.value || undefined);
    await nextTick();
    renderCharts();
  ...
  if (pieRef.value) {
    ...
    pieChart.setOption({
      ...
      series: [{
        ...
        radius: ['40%', '70%'],
        data: [
          { name: '出勤', value: s.present, itemStyle: { color: eduChartColors.success } },
          { name: '缺勤', value: s.absent, itemStyle: { color: eduChartColors.danger } },
          { name: '迟到', value: s.late, itemStyle: { color: eduChartColors.warning } },
          { name: '请假', value: s.leave, itemStyle: { color: eduChartColors.gray } },
        ],
      }],
    });
  }
...
async function onExport(format: 'excel' | 'pdf'): Promise<void> {
  ...
  try {
    await exportReport('attendance', format, { startDate: startDate.value || undefined, endDate: endDate.value || undefined });
